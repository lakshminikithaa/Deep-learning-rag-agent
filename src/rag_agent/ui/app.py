...
def render_chat_interface(graph, store: VectorStoreManager) -> None:
    ...
    with st.form("chat_query_form", clear_on_submit=True):
        query = st.text_input(
            "",
            placeholder="Ask about a deep learning topic...",
...
def main() -> None:
    ...
    if not st.session_state["ingested_documents"]:
        # Auto-ingest default corpus on first run if DB is empty
        stats = store.get_collection_stats()
        if stats.get("total_chunks", 0) == 0:
            ...
            if corpus_dir.exists():
                ...
                files_to_ingest = md_files + pdf_files
                if files_to_ingest:
                    with st.spinner("Auto-ingesting default corpus..."):
                        chunks = chunker.chunk_files(files_to_ingest)
                        store.ingest(chunks)
        refresh_documents(store)
